...
> **Forward-risk, flagged during Task 4's code review — read before implementing Tasks 6/7/8.** At the moment Task 4 runs, every `.muted`-classed element genuinely sits on raw `--bg` felt (verified: `roundHistory.js`'s empty-state paragraph, `scorer.js`'s "all rounds played" paragraph, `history.js`'s bid-accuracy caption, and all 5 of `setup.js`'s hint paragraphs) — so `--text-muted-on-bg` (sage, 5.92:1 on `--bg`) is correct today. But this stops being true once later tasks land their own `--surface` backgrounds:
> - **Task 7** adds `fieldset { background: var(--surface); }`. Four of `setup.js`'s five `.muted` paragraphs (`empty` in the players fieldset, `ruleHint` in the rules fieldset, and both `hint` variants in the hand-size fieldset) are children of a `<fieldset>` and will be swept onto parchment. Only `why` ("Select at least 2 players to start") is appended directly to `screen`, outside any fieldset, and stays safe.
> - **Task 8** adds `.history-screen section { background: var(--surface); }`. `history.js`'s bid-accuracy `caption` (`className = 'muted'`) is appended to a `<section>` inside the `history-screen` root, so it will be swept onto parchment too.
> - **Task 6**'s `.history__item { background: var(--surface); }` does NOT catch `roundHistory.js`'s empty-state `.muted` paragraph — that paragraph is appended to the wrapper `<section>` directly (an early return before any `.history__item` exists), not to a `.history__item`. `scorer.js`'s `.muted` "done" paragraph is likewise appended straight to `screen`, never inside `.scorer__alert`. Both remain on raw felt and are NOT at risk.
>
> Sage (`--text-muted-on-bg`, `#8FA88E`) on parchment (`--surface`, `#F4EDD9`) is only ~2.2:1 per Task 1's own palette notes — a class-level `color: var(--text-muted-on-bg)` beats the ancestor's inherited `color: var(--text)` at equal specificity (it's set directly on the element, not inherited), so simply adding the surface background will NOT fix the contrast on its own. **When implementing Task 7**, the four fieldset-nested `.muted` paragraphs need `--text-muted` (not `--text-muted-on-bg`) once they're inside a parchment fieldset — either give them a more specific override, or split `.muted` into page-level and card-level variants. **When implementing Task 8**, the same applies to `history.js`'s bid-accuracy caption. Re-run Task 4's grep-and-check against the *then-current* CSS before assuming `.muted`'s single shared rule still works everywhere it's used.
